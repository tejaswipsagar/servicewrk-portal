        ...
        <v-toolbar color="primary" class="white--text elevation-0" dense dark>
          ...
          <div class="ml-2">
            ...
            <span v-if="StoreObj.action != 'CREATE'">{{
              currentProductDetails[0]?.setting_name
            }}</span>
          </div>
          <v-spacer></v-spacer>
          <v-btn
...
export default {
  ...
  watch: {
    ...
    GetServiceByCategoriesList(val) {
      ...
      if (this.StoreObj.action == "EDIT") {
        this.products = [...this.productsList]
          .filter(
            (item) =>
              item.service_id == this.currentProductDetails[0].product_id
          )
          .map((ppp) => {
    ...
    async CreateUpdateProductFleetDialog(val) {
      if (val == true) {
        this.currentProductDetails =
          await this.getParticularProdFleetDataMethod(this.StoreObj.setting_id);
        ...
        if (this.StoreObj.action == "EDIT") {
          this.categories = this.currentProductDetails[0].category_id;
          this.products = this.currentProductDetails[0].product_id;
          this.GetServicesByCategoriesMethod(
            this.currentProductDetails[0].category_id
          );
          this.description = this.currentProductDetails[0].setting_description;
          this.product_manufactured_date =
            this.currentProductDetails[0].product_manufactured_date;
          this.asset_number = this.currentProductDetails[0].asset_number;
          this.software_version =
            this.currentProductDetails[0].software_version;
          this.organization_products_fleet_status =
            this.currentProductDetails[0].organization_products_fleet_status;
          this.serialNumber = this.currentProductDetails[0].setting_name;
          this.price = this.currentProductDetails[0].setting_price;
          this.service_center_id =
            this.currentProductDetails[0].service_center_name;
          this.s3_files =
            this.currentProductDetails[0].product_attachments != undefined &&
            this.currentProductDetails[0].product_attachments != null &&
          ...
          this.extended_warranty_details =
            this.currentProductDetails[0].extended_warranty_details !=
              undefined &&
            this.currentProductDetails[0].extended_warranty_details != null &&
            this.currentProductDetails[0].extended_warranty_details.length != 0
              ? this.currentProductDetails[0].extended_warranty_details
              : [];
        } else if (this.StoreObj.action == "EDIT_TO_MAKE_VALID") {
          this.is_customer_tagged = this.currentProductDetails[0].customer_name
            ? true
            : false;
          this.product_manufactured_date =
            this.currentProductDetails[0].product_manufactured_date != "-"
              ...
              : "";
          this.serialNumber = this.currentProductDetails[0].setting_name;
          this.invoice_number = this.currentProductDetails[0].invoice_number;
          this.product_purchased_on =
            this.currentProductDetails[0].product_purchased_on != "-"
              ? this.currentProductDetails[0].product_purchased_on
              : "";
          this.invoice_date =
            this.currentProductDetails[0].invoice_date != "-"
              ? this.currentProductDetails[0].invoice_date
              : "";
          this.price = this.currentProductDetails[0].price;
          this.asset_number = this.currentProductDetails[0].asset_number;
          this.software_version =
            this.currentProductDetails[0].software_version;
          this.category_name = this.currentProductDetails[0].category_name;
          // this.customer_id = this.StoreObj.customer_id;
          // this.customer_name = this.StoreObj.customer_name;
          this.description = this.currentProductDetails[0].description;
          this.frequency = this.currentProductDetails[0].frequency;
          ...
          this.warranty_end_date =
            this.currentProductDetails[0].warranty_end_date != "-"
              ? this.currentProductDetails[0].warranty_end_date
              : "";
          this.support_type = this.currentProductDetails[0].support_type;
          this.warranty_type = this.currentProductDetails[0].warranty_type;
          this.is_schedule_service = this.currentProductDetails[0].frequency
            ? true
            : false;
          this.organization_products_fleet_status =
            this.currentProductDetails[0].organization_products_fleet_status;
          ...
          this.customer_company_name =
            this.currentProductDetails[0].customer_company_name;
          this.customer_name = this.currentProductDetails[0].customer_name;
          this.serialNumber = this.currentProductDetails[0].setting_name;
          this.customer_id = this.currentProductDetails[0].customer_id;
          this.selected_files =
            this.currentProductDetails[0].product_attachment_file;
        }
        this.GetAllOrganizationSettingsTypesMethod();
      }
    },
  ...
  methods: {
    ...
    getFilterMethod() {
      this.selected = this.currentProductDetails[0].products_fleet_list;
    },
    callAPI() {
    ...
    async createEditProductBasedSerialNumberMethod(s3URL) {
      if (
        ...
        inputParams = {
          ...
          organization_id:
            this.$store.getters.get_current_user_details.organization_id,
          setting_id: this.currentProductDetails[0].setting_id,
          setting_name: this.serialNumber,
        };
        if (this.price != undefined && this.price != null && this.price != "") {
          inputParams.setting_price = this.price;
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          if (this.StoreObj.action == "EDIT_TO_MAKE_VALID") {
            try {
              var inputParams = {
                invalid_serial_id:
                  this.currentProductDetails[0].invalid_excel_id,
                organization_id:
                  this.$store.getters.get_current_user_details.organization_id,
              };
              let result = await API.graphql(
    ...
    async UpdateWorkFlowStatusToApprovedMethod() {
      ...
      var inputParams = {
        ...
        workflow_id: this.currentProductDetails[0].workflow_id,
