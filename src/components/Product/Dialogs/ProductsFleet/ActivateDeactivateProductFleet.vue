...
import { ActivateOrDeactivateSerialNumber } from "@/graphql/mutations.js";
    ...
    async activateInactivateMethod() {
      try {
        var inputParams = {
          ...
          action:
            this.StoreObj.setting_status == "ACTIVE"
              ? "DEACTIVATE"
              : "ACTIVATE",
        };
        this.loading = true;
        let result = await API.graphql(
          graphqlOperation(ActivateOrDeactivateSerialNumber, {
            input: inputParams,
          })
        );
        var ResultObject = JSON.parse(
          result.data.ActivateOrDeactivateSerialNumber
        );
