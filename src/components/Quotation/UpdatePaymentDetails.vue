<template>
  <div>
    ...
    <v-dialog
      :value="dialogUpdatePaymentDetails"
      persistent
      :width="
    ...
    >
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-btn icon @click="dialogUpdatePaymentDetailsEmit((Toggle = 1))"
        ...
        <v-card-actions class="pb-4">
          <v-spacer></v-spacer>
          <!-- <v-btn
            v-show="payment_method === '' || payment_method === 'PAYMENT_DONE'"
...
import { UpdateQuotationForPaymentDetails } from "@/graphql/mutations.js";
import { GenerateS3URL } from "@/mixins/Extras/GeneratePresignedUrl.js";
import Snackbar from "@/components/Extras/SnackbarView.vue";
export default {
  props: {
    StoreObj: Object,
    dialogUpdatePaymentDetails: Boolean,
  },
  components: {
    Snackbar,
  },
    ...
    async updatePaymentDetailsMethod() {
      try {
        ...
        let result = await API.graphql(
          graphqlOperation(UpdateQuotationForPaymentDetails, {
            input: inputParams,
          })
        );
        var ResultObject = JSON.parse(
          result.data.UpdateQuotationForPaymentDetails
        );
        if (ResultObject.status == "SUCCESS") {
          ...
          this.dialogUpdatePaymentDetailsEmit((this.Toggle = 2));
        }
