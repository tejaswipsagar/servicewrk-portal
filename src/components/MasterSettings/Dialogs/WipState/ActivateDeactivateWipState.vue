<template>
  <div>
    ...
    <v-dialog
      :value="ActivateDeactivateWipStateDialog"
      persistent
      max-width="400px"
    >
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-btn
            icon
            @click="ActivateDeactivateWipStateDialogEmit((Toggle = 1))"
...
export default {
  props: {
    StoreObj: Object,
    ActivateDeactivateWipStateDialog: Boolean,
  },
  components: {
    Snackbar,
  },
    ...
    async activateInactivateMethod() {
      ...
      try {
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.ActivateDeactivateWipStateDialogEmit((this.Toggle = 2));
        }
        this.loading = false;
      } catch (error) {
        this.loading = false;
