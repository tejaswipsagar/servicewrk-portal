<template>
  <div>
    ...
    <v-dialog
      :value="ActivateDeactivateAmcTypeDialog"
      persistent
      max-width="400px"
    >
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-btn icon @click="ActivateDeactivateAmcTypeDialogEmit((Toggle = 1))"
...
export default {
  props: {
    StoreObj: Object,
    ActivateDeactivateAmcTypeDialog: Boolean,
  },
  components: {
    Snackbar,
  },
    ...
    async activateInactivateMethod() {
      ...
      try {
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.ActivateDeactivateAmcTypeDialogEmit((this.Toggle = 2));
        }
        this.loading = false;
      } catch (error) {
        this.loading = false;
