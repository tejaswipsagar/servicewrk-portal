<template>
  <div>
    ...
    <v-dialog
      :value="ActivateInactivateProblemDescriptionDialog"
      persistent
      max-width="400px"
    >
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-btn
            icon
            @click="
              ActivateInactivateProblemDescriptionDialogEmit((Toggle = 1))
            "
...
export default {
  props: {
    ActivateInactivateProblemDescriptionDialog: Boolean,
    StoreObj: Object,
  },
  components: {
    Snackbar,
    ...
    async activateInactivateMethod() {
      ...
      try {
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.ActivateInactivateProblemDescriptionDialogEmit(
            (this.Toggle = 2)
          );
        }
        this.loading = false;
