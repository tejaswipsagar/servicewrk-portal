<template>
  <div>
    ...
    <v-dialog
      :value="ActivateInactivateCustomerCategoryDialog"
      persistent
      max-width="400px"
    >
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-btn
            icon
            @click="ActivateInactivateCustomerCategoryDialogEmit((Toggle = 1))"
...
export default {
  props: {
    ActivateInactivateCustomerCategoryDialog: Boolean,
    StoreObj: Object,
  },
  components: {
    Snackbar,
    ...
    async activateInactivateMethod() {
      ...
      try {
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.ActivateInactivateCustomerCategoryDialogEmit((this.Toggle = 2));
        }
        this.loading = false;
      } catch (error) {
        this.loading = false;
