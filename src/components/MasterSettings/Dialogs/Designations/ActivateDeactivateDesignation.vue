<template>
  <div>
    ...
    <v-dialog
      :value="ActivateDeactivateDesignation"
      persistent
      max-width="400px"
    >
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-btn icon @click="ActivateDeactivateDesignationEmit((Toggle = 1))"
...
export default {
  props: {
    ActivateDeactivateDesignation: Boolean,
    StoreObj: Object,
  },
  components: {
    Snackbar,
    ...
    async activateInactivateMethod() {
      ...
      try {
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.ActivateDeactivateDesignationEmit((this.Toggle = 2));
        }
        this.loading = false;
      } catch (error) {
        this.loading = false;
