<template>
  <div>
    ...
    />
    <v-dialog :value="UpdateProductDialog" persistent max-width="1200px">
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-btn icon @click="UpdateProductDialogEmit((Toggle = 1))"
          ...
          >
            <v-col cols="12" md="10" xs="12" sm="12">
              ...
              >
                ...
                <template v-slot:[`item.normal_warranty_date_list`]="{ item }">
                  <div v-if="item.normal_warranty_date_list">
                    <div v-if="item.normal_warranty_date_list.length != 0">
                      {{ item.normal_warranty_date_list.length }}
                      <v-icon
                        color="primary"
                        small
...
export default {
  ...
  data: () => ({
    ...
    frequency_items: [15, 30, 45, 60, 90, 120, 180, 365],
    normalSupportHeaders: [
      ...
      { text: "No. of Visits", value: "normal_warranty_date_list" },
        ...
        if (
          ...
          setTimeout(() => {
            this.supportTableItems.push({
              ...
              normal_warranty_date_list: this.rft_date_list,
            });
  ...
  methods: {
    ...
    changeFrequency() {
      ...
      this.supportTableItems[0].normal_warranty_date_list = generatedDates;
    },
    frequencyCalculate() {},
      ...
      if (item != undefined) {
        ...
        if (
          ...
          this.supportTableItems[item.index].ticket_frequency =
            item.ticket_frequency;
          this.supportTableItems[item.index].normal_warranty_date_list =
    ...
    async activateInactivateMethod() {
      ...
      if (
        ...
        this.supportTableItems[0].normal_warranty_date_list &&
        this.supportTableItems[0].normal_warranty_date_list.length > 0
      ) {
        inputParams.rft_date_list =
          this.supportTableItems[0].normal_warranty_date_list.map(
            (item) => item
          );
      }
      console.log("this.supportTableItems", this.supportTableItems);
      ...
      try {
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.EditedObject = {};
          this.EditedSupportObject = {};
          this.UpdateProductDialogEmit((this.Toggle = 2));
        }
        this.loading = false;
      } catch (error) {
        this.loading = false;
    ...
    UpdateProductDialogEmit(Toggle) {
      this.s3_files = [];
      this.selected_files = [];
