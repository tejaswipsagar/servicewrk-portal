      ...
      <v-row no-gutters>
        <v-col cols="12" sm="12" md="3" xs="12">
          <v-toolbar dense class="elevation-0">
            <div class="largeFontSizeNew">Create Contract</div>
          </v-toolbar>
        </v-col>
              ...
              <v-col cols="12" md="12">
                <v-card outlined class="ml-n1">
                  ...
                  <v-row class="mx-0 mt-2">
                                    ...
                                    >
                                      ...
                                      <v-card>
                                        ...
                                        <v-card-subtitle
                                          ...
                                          align="center"
                                        >
                                          Set Height and width of Header Image
                                        </v-card-subtitle>
                    ...
                    <v-col v-if="cust_shown == true" cols="12" md="12">
                      <v-select
                        outlined
                        dense
...
export default {
  ...
  mounted() {
    ...
    // this.GetAllOrganizationSettingsMethod(
    //   (this.status = "ACTIVE"),
    //   (this.setting_type = "AMC")
    // );
  },
  watch: {
    select_template(item) {
      ...
      for (let i = 0; i < this.StoreObj.html_array.length; i++) {
        this.contentArray[i] = this.StoreObj.html_array[i].html_contents;
      }
      this.content = this.contentArray;
      if (this.content.includes("$_{Organization_Address}")) {
        this.org_shown = true;
      } else {
        this.org_shown = false;
      }
      if (this.content.includes("$_{Header}")) {
        this.header_shown = true;
      } else {
        this.header_shown = false;
      }
      if (this.content.includes("$_{Footer}")) {
        this.footer_shown = true;
      } else {
        this.footer_shown = false;
      }
      if (this.content.includes("$_{Customer_Name}")) {
        this.cust_shown = true;
      } else {
        this.cust_shown = false;
      }
      if (this.content.includes("$_{Date_Selection}")) {
        this.date_shown = true;
      } else {
        this.date_shown = false;
      }
      if (this.content.includes("$_{Quotation_Table}")) {
    ...
    customer_address(val) {
      ...
      // this.content = this.content.includes("$_{Organization_Address}")
      //   ? this.content.replace("$_{Organization_Address}", val)
      //   : this.content;
    },
    customer_name(val) {
          ...
          fr.addEventListener("load", async () => {
            ...
            try {
              ...
              if (self.urls3.length == self.fileListFooter.length) {
                // self.create_category_api(self.urls3);
                callbackFn(self.urls3);
                // return self.urls3;
              }
            } catch (error) {
      ...
      try {
        ...
        for (let i = 0; i < this.StoreObj.html_array.length; i++) {
          new_html_contents_array.push({
            html_contents: `<div style="white-space: pre"> ${this.StoreObj.html_array[i].html_contents}</div><style>
            .ql-align-center {
              text-align: center;
            }
            .ql-align-left {
