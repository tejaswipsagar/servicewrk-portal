<template>
  <div>
    ...
    <v-dialog v-model="UpdatePaymentDetailsDialog" persistent :width="1000">
      <v-card>
        <v-toolbar dark dense class="elevation-0" color="primary">
          <div>Update Payment Details</div>
          <v-spacer></v-spacer>
          <v-btn icon @click="UpdatePaymentDetailsDialogEmit((Toggle = 1))"
...
import { UpdatePurchaseOrderPaymentDetails } from "@/graphql/mutations.js";
import { GenerateS3URL } from "@/mixins/Extras/GeneratePresignedUrl.js";
import Snackbar from "@/components/Extras/SnackbarView.vue";
export default {
  props: {
    StoreObj: Object,
    UpdatePaymentDetailsDialog: Boolean,
  },
  components: {
    Snackbar,
  },
  ...
  methods: {
    ...
    async uploadMediaToS3() {
      ...
      await this.updatePaymentDetailsMethod();
    },
    ...
    async updatePaymentDetailsMethod() {
      ...
      try {
        ...
        let result = await API.graphql(
          graphqlOperation(UpdatePurchaseOrderPaymentDetails, {
            input: inputParams,
          })
        );
        var ResultObject = JSON.parse(
          result.data.UpdatePurchaseOrderPaymentDetails
        );
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.UpdatePaymentDetailsDialogEmit((this.Toggle = 2));
        }
