<template>
  <div>
    <v-dialog
      :value="taggedTerritoriesDialog1"
      scrollable
      persistent
      :overlay="false"
      max-width="80%"
      ...
      <v-card>
        <v-toolbar elevation="0" dense color="primary">
          <strong class="white--text"
            ...
            {{ StoreObj.closed_ticket_count }}</strong
          >
          <v-spacer />
        ...
        <v-card-text class="py-0 mt-2">
          <v-data-table
            dense
            :headers="headers"
            :items="StoreObj.closed_ticket_details"
            :footer-props="{
          ...
          >
            ...
            <template v-slot:[`item.ticket_closed_on`]="{ item }">
              <div class="FontSize" v-if="item.ticket_closed_on != undefined">
                {{
                  new Date(item.ticket_closed_on * 1000).toLocaleString("en-GB")
                }}
              </div>
              <div v-else>-</div>
            </template>
...
export default {
  ...
  props: {
    StoreObj: Object,
    taggedTerritoriesDialog1: Boolean,
  },
  data: () => ({
    headers: [
      ...
      { text: "Closed On", value: "ticket_closed_on" },
    ],
  }),
  methods: {
    closeEmmit2(Toggle) {
    ...
    DownloadExcel() {
      this.StoreObj.closed_ticket_details.map((item) => {
        item.updated_ticket_closed_on = new Date(
          item.ticket_closed_on * 1000
      ...
      let data = [
        {
          ...
          columns: [
            ...
            {
              label: "Ticket Closed on",
              value: "updated_ticket_closed_on",
            },
          ],
          content: this.StoreObj.closed_ticket_details,
        },
      ];
      let settings = {
        fileName: "Closed Ticket Details",
        extraLength: 3,
        writeOptions: {},
      };
      xlsx(data, settings);
