    ...
    <v-card class="elevation-0">
      <v-toolbar dense class="elevation-0">
        ...
        <v-btn
          small
          color="primary"
          @click="ConvertTicketNewCustomerEmit((Toggle = 1))"
...
export default {
  ...
  async mounted() {
    if (this.GetCustomerDetails.length === 0) {
      this.autoFetchDetailsMethod();
    }
    console.log("STOREOBJ", this.StoreObj);
    console.log("STOREOBJ1", this.StoreObj1);
    ...
    this.create_ticket.source_remark =
      this.StoreObj.call_source_remarks || undefined;
    this.create_ticket.customer_call_time = this.StoreObj.customer_call_time;
    this.create_ticket.customer_call_date = this.StoreObj.customer_call_date;
    this.create_ticket.ticket_call_source =
      this.StoreObj.ticket_call_source || undefined;
    setTimeout(() => {
      this.GetAllUerTypeLocationBasedUsers(
        this.create_ticket.ticket_latitude,
        this.create_ticket.ticket_longitude,
    ...
    if (
      ...
      if (
        ...
        if (supportEndDateInsecs < currentEpochTimeInSeconds) {
          ...
          setTimeout(() => {
            this.create_ticket.warranty_type =
              this.StoreObj.warranty_type || this.StoreObj.support_type;
          }, 1000);
          this.create_ticket.warranty_start_date =
  ...
  methods: {
    autoFetchDetailsMethod() {
      const obj = {
        customer_pincode: this.StoreObj.ticket_pincode,
        customer_company_name: this.StoreObj.customer_company_name,
        customer_type: this.StoreObj.customer_type,
        customer_primary_country_code: this.StoreObj.customer_country_code,
        customer_primary_contact_number: this.StoreObj.customer_phone_number,
        customer_address: this.StoreObj.ticket_address,
        city: this.StoreObj.ticket_city,
        state: this.StoreObj.ticket_state,
        customer_latitude: this.StoreObj.ticket_latitude,
        customer_longitude: this.StoreObj.ticket_longitude,
      ...
      this.GetCustomerDetails.push(obj);
      console.log("GetCustomerDetails", this.GetCustomerDetails);
    },
    ...
    async CreateAssignedTicket(s3URL) {
      ...
      try {
        ...
        var inputparams = {
          ...
          is_customer_initiated_ticket:
            this.StoreObj.selected_tab === "Customer Initiated"
              ? true
              : undefined,
          customer_initiated_ticket_id:
            this.StoreObj.selected_tab === "Customer Initiated"
              ? this.StoreObj.request_for_ticket_id
              : undefined,
          ticket_id:
        ...
        if (ResultObject.status == "SUCCESS") {
          ...
          this.ConvertTicketNewCustomerEmit(
            (this.Toggle = 2),
    ...
    ConvertTicketNewCustomerEmit(Toggle, status_message) {
