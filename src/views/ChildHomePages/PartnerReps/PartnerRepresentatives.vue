  ...
  <div>
    ...
    <v-card class="elevation-0">
      <v-toolbar dense class="elevation-0">
        <div class="largeFontSizeNew">Representatives</div>
        <v-spacer />
        <v-select
          dense
          outlined
        ...
        <v-tooltip color="primary" bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              @click.stop="CreatePartnerRepsMethod()"
              small
              v-on="on"
              v-bind="attrs"
          ...
          <span class="white--text">Add Representative</span>
        </v-tooltip>
      </v-toolbar>
      <v-card-text>
        <v-data-table
        ...
        >
          ...
          <template v-slot:[`item.Actions`]="{ item }">
            ...
            <v-tooltip bottom v-if="item.user_status != 'INVITED'">
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  v-bind="attrs"
                  v-on="on"
            ...
            >
              ...
              <span>Delete {{ item.user_name }}</span>
            </v-tooltip>
          </template>
        </v-data-table>
      </v-card-text>
...
export default {
  ...
  data: () => ({
    ...
    Headers: [
      ...
      { text: "Phone", value: "user_full_number" },
  ...
  methods: {
    CreatePartnerRepsMethod() {
      this.StoreObj.action = "CREATEREP";
      this.StoreObj.action2 = "CREATE_PARTNER_REP";
