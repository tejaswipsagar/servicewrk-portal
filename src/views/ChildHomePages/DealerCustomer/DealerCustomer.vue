  ...
  <div>
    ...
    <v-card-text v-if="current_view === 'LIST'">
      <v-toolbar dense class="elevation-0">
        <div class="largeFontSizeNew py-0 ml-n1 mt-1">Customers</div>
        <v-spacer />
        <v-form ref="form">
              ...
              <v-tooltip color="primary" bottom>
                <template v-slot:activator="{ on, attrs }">
                  <span v-on="on" v-bind="attrs">
                    <v-btn
                      small
                      @click="downloadCustomerTempMethod()"
                      class="primary white--text text-capitalize ml-2"
                    >
                ...
                <span>Download Bulk Upload Customer Template</span>
              </v-tooltip>
              <v-tooltip color="primary" bottom>
                <template v-slot:activator="{ on, attrs }">
                  <span v-on="on" v-bind="attrs">
      ...
      <v-data-table
        ...
        :items="
          isSearching === true ? GetAllSearchCustomerList : GetAllCustomersData
        "
        :no-data-text="
          GetAllCustomersData.length === 0 ? 'No data available' : ''
        "
      >
        <template
...
import { ListDealerCustomerData } from "@/mixins/Dealers/Customers/GetDealerCustomerList";
export default {
  mixins: [
    ListDealerCustomerData,
    GenerateS3URL,
    GetAllCustomersList,
  ...
  watch: {
    async customer_type(val) {
      if (val) {
        ...
        this.GetAllCustomersData = await this.ListDealerCustomerDataMethod(
          val,
      ...
      setTimeout(() => {
        generateExcel()
          .then((buffer) => {
            ...
            downloadLink.download = `TEMPLATE_FOR_CUSTOMER_BULK_UPLOAD-${new Date().toLocaleString(
              "en-GB"
            )}.xlsx`;
            downloadLink.click();
          })
