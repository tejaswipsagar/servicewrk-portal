<template>
  <div>
    ...
    <CreateCustomersCard
      ...
      @clicked="CreateCustomerCardEmit"
    />
    <CustomerTabCard
      ...
      @clicked="EmitTab"
    />
    <v-card-text v-if="current_view == 'LIST' && customerDataLoaded">
        ...
        <div class="largeFontSizeNew py-0 ml-n1 mt-1">
          ...
          {{
            SearchBy !== "all" && search !== ""
              ...
              ? customerData.length
              : GetAllCustomerInvalidList.length
          }}
          <v-btn
            small
            v-show="showCreateButton"
            class="primary white--text ml-2"
            @click="(current_view = 'CREATE'), (StoreObj.action = 'CREATE')"
            >Create Customer<v-icon>mdi-plus</v-icon></v-btn
          >
          <!-- {{
            customer_status !== "INVALID"
              ? customerData.length
      ...
      <v-data-table
        dense
        class="mt-2 TableValFontsize"
        fixed-header
        :page.sync="page"
        :headers="customer_status === 'INVALID' ? invalidHeaders : headers"
        :items-per-page="50"
        :height="TableHeight"
      ...
      >
        ...
        <template v-slot:[`item.customer_company_name`]="{ item }">
          ...
          <div v-if="customer_status === 'INVALID'">
            <div class="FontSize">
              {{ item.customer_name }}
            </div>
            <div
        ...
        <template v-slot:[`item.customer_category_name`]="{ item }">
          <div class="FontSize" v-if="item.customer_category_name">
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Contact Person Name", value: "customer_name" },
      // { text: "Category", value: "customer_category_name" },
      { text: "Phone", value: "customer_primary_contact_number" },
  ...
  watch: {
    search(val) {
      if (val.length > 2) {
        ...
        if (this.SearchBy !== "all") {
          ...
          this.filteredCustomerData = this.customerData.filter((obj) =>
    ...
    page() {
      if (
        ...
        if (this.customer_status != "INVALID") {
          this.GetAllCustomersListMethod(
            this.customer_status,
            this.customer_type,
            this.next_token
          );
        } else if (this.customer_status == "INVALID") {
          this.GetAllInvalidCustomersMethod(this.next_token);
        }
      }
    },
    ...
    async customer_status(val) {
      ...
      // await this.customerApiCallingMethod();
      if (val === "ACTIVE" || val === "INACTIVE") {
        this.customerData = allCustomers.filter(
          (item) =>
            item.customer_status === val &&
            item.customer_type === this.customer_type
        );
      } else {
    ...
    async customer_type(val) {
      ...
      if (val === "INDIVIDUAL" || val === "BUSINESS") {
        this.customerData = allCustomers.filter(
          (item) =>
            item.customer_type === val &&
            item.customer_status === this.customer_status
        );
      } else {
        this.customerData = allCustomers;
      }
    },
  },
  async mounted() {
    this.overlay = true;
    // const allCustomers = await this.getAllCustomerUsingS3URLMethod();
    // const findedItem = allCustomers.find(
    //   (item) => item.customer_primary_contact_number === 9304006232
    // );
    // console.log("FINDED_9304006232", findedItem);
    ...
    const a = this.$route.params.call_rfs_data;
    if (
  ...
  methods: {
    ...
    //   return this.customerData.filter((obj) =>
    //     Object.values(obj).some((val) =>
    //       String(val).toLowerCase().includes(lowerSearch)
    //     )
    //   );
    // },
    async customerApiCallingMethod() {
      this.GetCurrentUser();

      const s3UrlData = this.$store.getters.get_s3_url_data;
      const customerS3UrlKey =
        this.$store.getters.get_current_user_details.organization_details
          .version_id;
      if (s3UrlData.customer_data?.customer_key) {
        if (customerS3UrlKey === s3UrlData.customer_data.customer_key) {
          console.log("KEYS1", customerS3UrlKey);
          console.log("KEYS2", s3UrlData.customer_data.customer_key);
          this.customerData = s3UrlData.customer_data.customer_data_list;
          console.log("NO_CALLING");
        } else {
          this.customerData = await this.getAllCustomerUsingS3URLMethod();
