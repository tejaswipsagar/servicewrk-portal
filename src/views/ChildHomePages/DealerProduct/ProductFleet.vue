<template>
  <div>
    ...
    />
    <v-card elevation="0" v-if="currentView === 'PRODUCT_FLEET_LIST_VIEW'">
            ...
            <v-menu right bottom>
              ...
              <v-list dense nav>
                <v-list-item-group color="primary">
                  ...
                  <v-list-item
                    v-if="
                      item.product_status === 'CUSTOMER_TAGGED' &&
                      $store.getters.get_current_user_details
                        .enable_ticket_creation_for_product_fleet
                    "
                    @click="createTicketMethod(item)"
                  >
                    <v-list-item-icon>
                    ...
                    <v-list-item-content>
                      <v-list-item-title>Raise a complaint</v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                  <!-- <v-list-item
                      v-else
    ...
    <CreateTicket
      v-if="currentView === 'CREATE_TICKET_VIEW'"
      :StoreObj="storeObj"
...
export default {
  ...
  data: () => ({
    ...
    dealerProductItems: [],
    selectedCustomer: "",
    currentView: "PRODUCT_FLEET_LIST_VIEW",
    customerItems: [],
    productsHeaders: [
  ...
  methods: {
    ...
    createTicketMethod(item) {
      this.currentView = "CREATE_TICKET_VIEW";
      this.storeObj = item;
      this.storeObj.action = "CREATE_TICKET";
      this.storeObj.create_ticket_from = "DEALER_PORTAL";
    },
    ...
    async CreateTicketCardEmit(Toggle) {
      this.currentView = "PRODUCT_FLEET_LIST_VIEW";
      if (Toggle === 2) {
